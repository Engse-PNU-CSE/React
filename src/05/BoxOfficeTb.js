...
export default function BoxOfficeTb({boxList, setSelMv}) {
  const handleClick = (mv) => {
    setSelMv(`[${mv.movieNm}] Total Sales : ${parseInt(mv.salesAcc).toLocaleString()}원 , Total Audience : ${parseInt(mv.audiAcc).toLocaleString()}명`);
  }
  const handleoff = () => {
    setSelMv(`Select The Movie`)
  }

  const trs = boxList.map( item => 
                <tr key={item.movieCd}
                    onMouseEnter={() => {handleClick(item)}} onMouseLeave={handleoff}
                    className="h-10 p-2 hover:bg-blue-100 hover:font-bold">
                  <td className="text-center">
                    <span className=" inline-flex w-3/5 p-1 justify-center items-center bg-blue-900 text-white text-sm">
                      {item.rank}
                    </span>
                  </td>
                  <td>
