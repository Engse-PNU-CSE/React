...
export default function Movies({rank, movieNm, salesAcc, audiAcc, rankInten}) {
  ...
  const ontable = () => {  
      setmovies(`[${movieNm}] Total Sales : ${parseInt(salesAcc).toLocaleString()}원 , Total Audience : ${parseInt(audiAcc).toLocaleString()}명`);
  ...
  return (
    <tr key = {movieNm} className="bg-white hover:bg-slate-400" onMouseEnter={leavetable} onMouseLeave={ontable}>
        <th>{rank} 위</th>
        <td>{movieNm}</td>
